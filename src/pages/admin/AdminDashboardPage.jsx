        ...
        <div className="admin-layout">
          ...
          <div className="admin-content">
            <div className="grid-4" style={{ marginBottom: 'var(--space-8)' }}>
              <StatsCard icon={<FiUsers size={22} />} label={t('admin.totalUsers')} value={stats?.stats?.totalUsers || 0} />
              <StatsCard icon={<FiHome size={22} />} label={t('admin.activeListings')} value={stats?.stats?.activeListings || 0} color="#10B981" bgColor="#ECFDF5" />
              <StatsCard icon={<FiClock size={22} />} label={t('admin.pendingListings')} value={stats?.stats?.pendingListings || 0} color="#F59E0B" bgColor="#FFFBEB" />
              <StatsCard icon={<FiShield size={22} />} label={t('admin.pendingVerifications')} value={stats?.stats?.pendingVerifications || 0} color="#0EA5E9" bgColor="#F0F9FF" />
            </div>
            <div className="grid-2">
              ...
              <div className="card" style={{ padding: 'var(--space-6)' }}>
                ...
                <div className="flex-col gap-3">
                  <div className="flex items-center justify-between"><span style={{ color: 'var(--text-muted)' }}>{t('admin.students')}</span><span style={{ fontWeight: 700 }}>{stats?.stats?.totalStudents || 0}</span></div>
                  <div className="flex items-center justify-between"><span style={{ color: 'var(--text-muted)' }}>{t('admin.owners')}</span><span style={{ fontWeight: 700 }}>{stats?.stats?.totalOwners || 0}</span></div>
                  <div className="flex items-center justify-between"><span style={{ color: 'var(--text-muted)' }}>Total Listings</span><span style={{ fontWeight: 700 }}>{stats?.stats?.totalListings || 0}</span></div>
                </div>
              </div>
            </div>
          </div>
